...
const Settings = ({ closeMenu, settingsState, dispatch }) => {
  ...
  return (
    <div
      style={{ background: "#616062" }}
      className={`SettingsMenu p-10 h-full w-1/4 absolute right-0 z-10 ${transform}`}
    >
      <div className="flex justify-between">
        <div className="py-2.5 text-white text-4xl">Settings</div>
        <Button
          onClick={closeMenu}
          className="pointer-events font-extrabold text-white left-5 top-2 uppercase rounded"
        >
          <img
            src={SettingsCheckIcon}
            alt="Settings close button"
            className="inline-block"
          />
        </Button>
      </div>
      <div className="flex justify-start pt-16 pb-8">
        <div style={{ color: "#212121" }} className="font-bold text-xl">
          FEATURES
        </div>
      </div>
      <div className="flex justify-between">
